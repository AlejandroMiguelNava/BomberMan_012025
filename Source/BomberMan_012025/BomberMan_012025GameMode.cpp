...
void ABomberMan_012025GameMode::SpawnBloque(FVector posicionBloque,  int32 tipoBloque)
{
	...
	if (CharacterGenerado) //agregar el enemigo al TArray si fue generado
	{
		aEnemigos.Add(CharacterGenerado);	
	}
}
...
void ABomberMan_012025GameMode::UbicarJugadorAleatorio()
{
	if (ABomberMan_012025Character* PlayerCharacter = Cast<ABomberMan_012025Character>(UGameplayStatics::GetPlayerCharacter(GetWorld(), 0)))
	{
		...
		{
			fila = FMath::RandRange(0, aMapaBloques.Num() - 1);
			columna = FMath::RandRange(0, aMapaBloques[fila].Num() - 1);
		}
